biocinstallRepos <- function()
{
    ...
    repos <- paste("http://bioconductor.org/packages/2.1", repos, sep="/")
...
### biocinstall() version 2.1.11
### Called by biocLite() when R version 2.6 is detected.
###
                        ...
                        dependencies=c("Depends", "Imports"), type=getOption("pkgType"), ...)
{
    ...
    VERSION <- "2.1.11"
    ## R version 2.6 should have been detected by biocLite()
    ...
    cat("Your version of R requires version 2.1 of BioConductor.\n")

	if (type == "mac.binary" && substring(R.Version()[["os"]], 1, 8) != "darwin8.") {
		cat("WARNING:  The Mac binary versions of BioConductor 2.1 packages are built for Mac OS X Tiger.\n",
