...
### chunk number 14: 
###################################################
randiv <- function(x.train, y.train)
  {
    ## Repeat all the code from section 1.3
...
for(r in 1:runs)
 {
   results         <- randiv(x.train, y.train)
...
### chunk number 20: 
###################################################
yt.num <- as.numeric(y.train)-1
f.stat <- mt.teststat(t(x.train), yt.num, test = "f")
b.gene <- rev(order(f.stat))[1:200]
x.test <- x[c(1:10), b.gene]
y.pred <- stat.diag.da(x.train[,b.gene], yt.num, x.test)
y.pred
...
### chunk number 21: 
###################################################
y.pred <- factor(y.pred$pred, labels = levels(y))
y.test <- y[1:10]
table(y.pred, y.test)
