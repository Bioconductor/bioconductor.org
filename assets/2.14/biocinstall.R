...
### biocinstall() version 2.9.3
### Called by biocLite() when R version >= 2.14.0 is detected.
###
...
biocinstall <- function(pkgs, groupName="default", repos, ...)
{
    ...
    VERSION <- "2.9.3"               # this script version
    ...
    cat("Using R version ", thisRVer, " (R-devel), ",
        "biocinstall version ", VERSION, ".\n", sep="")

    stopifnot(require("utils"))

    if (compareVersion(thisRVer, NEXT_R_DEVEL_VERSION) >= 0)
        cat("WARNING: temporarily using Bioconductor version", BIOC_VERSION,
            "until the next\n         Bioconductor development cycle starts.\n")
