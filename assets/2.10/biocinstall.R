biocinstallRepos <- function()
{
    ...
    mirror <-
      as.character(getOption("BioC_mirror", "http://www.bioconductor.org"))
    repos <- paste(mirror, "packages", repos, sep="/")
   c(repos, "http://brainarray.mbni.med.umich.edu/bioc", "http://cran.fhcrc.org")
   c(repos, ifelse(getOption("repos")=="@CRAN@", "http://cran.fhcrc.org", getOption("repos")))
}
...
biocinstallPkgGroups <- function(groupName="default")
{
    ...
    affy_group <- c(
        ...
        "gcrma", "makecdfenv", "marray", "affyQCReport")
    if (groupName == "affy")
        return(affy_group)
...
### biocinstall() version 2.5.11
### Called by biocLite() when R version 2.10 is detected.
###
...
biocinstall <- function(pkgs, groupName="default", repos, ...)
{
    ...
    VERSION <- "2.5.11"        # this script version
    BIOC_VERSION <- "2.5"      # this version of Bioconductor
    ...
    if (type %in% c("mac.binary", "mac.binary.leopard")) {
        ...
        if (length(mbniPkgs) > 0) {
            ...
            warning(paste("MBNI Brain Array packages (", mbniList, ") are not available ",
                          "as Mac binaries. Please use biocLite again with type='source'.",
                          sep=""))
        }
    }
