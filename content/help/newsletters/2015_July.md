...
package. 'biocViews' terms are arranged in a hierarchy; each term can have one 
or more child terms. 'recommendPackages()' returns packages tagged with a 
particular term(s) as well as those tagged with child terms.

The function is available in the development branch (`Bioconductor` 3.2) only,
however, both release and development repositories can be searched by
changing the 'use.release' argument.
