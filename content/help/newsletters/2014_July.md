...
The `Bioconductor` home page now has what we hope
...
and suggested possible terms for use in the `biocViews:` field of
...
encouraged to run it before submitting
a package. Remember that biocViews are case-sensitive and branch-sensitive 
(i.e., terms for a Software package must come from the Software 
branch of biocViews). 
...
to traditional vignettes and work flows.
...
Historically, only `Bioconductor` _software_ packages were maintained
in a repository with a distinct branch for each release. Experimental
data and annotation packages had a trunk with no branches.

The experiment data packages now have a distinct release branch. This
allows software and experimental data packages to evolve together in
release and devel build environments.  It was often the case that
updates to a software package broke the companion experimental
data. Changes made to the experimental data were committed to trunk
and propagated to both release and devel builds, creating
incompatibilities in one place or the other.

A consequence of creating the new branch is the need to bump 'y' of
the 'x.y.z' version at release time (as we do for software). The
[version numbering scheme](/developers/how-to/version-numbering) is
the same as for software packages.Annotations remain the same with the
single trunk repository and no y bump.
...
Topics this year included RNASeq differential expression, variant
calling and ChIP-Seq as well as the essentials of statistical testing,
machine learning, visualization and of course using R.  Michael
Lawrence presented a `Scalable Genomics` lab which covered topics of
limiting resource consumption, using iteration when appropriate, and
scaling genome graphics. Much of the material is based on
...
This online course was offered by Rafael Irizarry and Michael Love.
Course goals were to enable students to 
