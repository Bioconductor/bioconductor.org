...
Now you're ready to spin up an MPI cluster. Use the <code>mpiutil</script>. Invoked without arguments, 
...
* For "vol-9999999", substitute the volume ID generated by the <code>create_sample_volume</code> script.
...
tasks with the following command:

	mpiutil -a XXX -s YYY -n workers -h -v vol-9999999

Make the following substitutions:
* "XXX" should be your Amazon Access Key ID
* "YYY" should be your Amazon Secret Key
